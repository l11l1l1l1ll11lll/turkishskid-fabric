...
public abstract class ItemRendererMixin {
    ...
    private static void renderItem$renderBakedItemModel(BakedModel model, int[] tints, int light, int overlay, MatrixStack matrices, VertexConsumer vertexConsumer, Operation<Void> original, @Local(argsOnly = true) ModelTransformationMode renderMode) {
        EntityModifierModule module = Lyrica.MODULE_MANAGER.getModule(EntityModifierModule.class);
        ...
        for (Direction direction : Direction.values()) {
            random.setSeed(42L);
            lyrica$renderBakedItemQuads(matrices, vertexConsumer, model.getQuads(null, direction, random), tints, color, light, overlay);
        }

        random.setSeed(42L);
        lyrica$renderBakedItemQuads(matrices, vertexConsumer, model.getQuads(null, null, random), tints, color, light, overlay);
    }

    @Unique
    private static void lyrica$renderBakedItemQuads(MatrixStack matrices, VertexConsumer vertexConsumer, List<BakedQuad> quads, int[] tints, Color color, int light, int overlay) {
